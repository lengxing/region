...
Region.prototype.create = function(options) {
     ...
     */
    var properties = {
        regionData: _.clone(Region.REGION_LIST),
        mode: 'multi',
        pureSelect: true,
        rawValue: []
    };
    ...
    selectMulti(this, properties.rawValue);
 ...
 * @override
 */
Region.prototype.initEvents = function() {
    $(this.main).delegate('input,label', 'click', this, mainClick);
    // 鼠标悬浮事件
    $(this.main).delegate(".ui-region-province-box", 'mouseover', this, _.partial(mainMouseHandler, 'show'));
    // 鼠标悬浮事件
    $(this.main).delegate(".ui-region-province-box", 'mouseout', this, _.partial(mainMouseHandler, 'hide'));
};
 ...
 * @return {Array}
 */
Region.prototype.getRawValue = function() {
    if (this.pureSelect) {
        // 伪造一个
        var node = {
            // 应该没人用这么奇葩的id吧。。。
 ...
 * @return {string}
 */
Region.prototype.stringifyValue = function(rawValue) {
 ...
 * @return {*}
 */
Region.prototype.parseValue = function(value) {
    return value.split(',');
};
 ...
 * @public
 */
Region.prototype.checkRegion = function(id) {
...
function createMultiRegion(region) {
    ...
    html.push(lib.format(
        tpl, {
            inputId: "ui-region-param-value",
            name: "ui-region-param-value"
        }
    ));
    $(region.main).append(html.join(''));
}
...
function getLevelHtml(region, item, level) {
    ...
    switch (level) {
        ...
        case 1:
            return lib.format(
                tplBoxWrapper, {
                    boxClass: 'ui-region-country-box',
                    itemClasses: 'ui-region-country-check',
                    itemWrapperId: '',
                    itemValue: item.id,
                    itemId: 'ctrl-default-test-item' + item.id,
                    level: item.level,
                    text: item.text,
                    contentClass: '',
                    content: subItemHtml.join(''),
                    checkboxCustomClass: "ui-checkbox-custom"
                }
            );
        ...
        case 2:
            return lib.format(
                tplBoxWrapper, {
                    boxClass: 'ui-region-region-box' + (tempIdx++) % 2,
                    itemClasses: 'ui-region-region-check',
                    itemWrapperId: '',
                    itemValue: item.id,
                    itemId: 'ctrl-default-test-item' + item.id,
                    level: item.level,
                    text: item.text,
                    contentClass: 'ui-region-province-box',
                    content: subItemHtml.join(''),
                    checkboxCustomClass: "ui-checkbox-custom"
                }
            );
        ...
        case 3:
            var layer = lib.format(
                tplPopLayer, {
                    popLayerClass: 'ui-region-locator',
                    layerBoxClass: 'ui-region-city-box',
                    hiddenClass: "state-hidden",
                    id: 'ctrl-default-test-sub-' + item.id,
                    infoId: 'ctrl-default-test-info-' + item.id,
                    innerHTML: subItemHtml.join('')
                }
            );
            var text = lib.format(
                tplInputItem, {
                    itemClasses: 'ui-region-text',
                    itemWrapperId: 'ctrl-default-test-wrapper-' + item.id,
                    itemValue: item.id,
                    itemId: 'ctrl-default-test-item' + item.id,
                    level: item.level,
                    text: item.text,
            ...
            return lib.format(
                tplProvinceWrapper, {
                    classes: 'ui-region-province-item',
                    content: text + layer
                }
            );
        // 市（区）
...
function getOptionDOM(region, id) {
    return lib.g('ctrl-default-test-item' + id);
}
...
function updateParamValue(region) {
    var input = lib.g("ui-region-param-value");
...
function updateSelectedTip(region, selCityIdsLength, cLength, id) {
    var $infoTag = $(lib.g("ctrl-default-test-info-" + id));
    if (selCityIdsLength !== 0 && selCityIdsLength !== cLength) {
        $infoTag.removeClass("state-hidden");
        $infoTag.html(selCityIdsLength + '/' + cLength + '');
    } else {
        $infoTag.addClass("state-hidden");
        $infoTag.html('');
    }
}
...
function mainMouseHandler(type, e) {
    ...
    var tar = e.target;
    var textClass = "ui-region-text";
    var layerClass = "ui-region-city-box";
    ...
    while (tar && tar !== document.body) {
        ...
        if (optionChildLayer) {
            handler(e.data, optionChildLayer, itemId);
            return;
        }
...
function showSubCity(region, dom, itemId) {
    ...
    $(dom).removeClass("state-hidden");
    var wrapper = dom.parentNode.previousSibling;
    $(wrapper).addClass('ui-region-text-over')
}
...
function selectMulti(region, values) {
    ...
    // 把整理后全数据做标识
    var map = {};
    _.each(fullValues, function(value) {
        map[value] = 1;
    });

    // 根据标识勾选
    _.each(region.regionDataIndex, function(item, key) {
...
function hideSubCity(region, dom, itemId) {
    $(dom).addClass("state-hidden");
    var wrapper = dom.parentNode.previousSibling;
    $(wrapper).removeClass('ui-region-text-over')
}
